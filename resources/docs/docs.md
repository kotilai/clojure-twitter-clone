...
The `home-routes` handler in the `clojure-twitter-clone.routes.home` namespace
...
The `home-page` function will in turn call the `clojure-twitter-clone.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/clojure_twitter_clone/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `clojure-twitter-clone.handler` namespace:
...
Request middleware functions are located under the `clojure-twitter-clone.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `clojure-twitter-clone.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `clojure-twitter-clone.db.core` in some other namespace.
